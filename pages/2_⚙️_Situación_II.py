...
queue_df = pd.DataFrame(columns=["Hora actual", "Evento", "Clientes en cola", "Hora sig. llegada",
                                 "Hora sig. fin de servicio", "Hora sig. inicio de descanso",
                                 "Hora sig. fin de descanso", "Estado de servicio"])

# Definir función para manejar llegadas
def handle_arrival(time, queue, arrival_interval, break_frequency, break_duration, service_active):
    """Añade un cliente a la cola cuando el evento es de llegada."""
    queue.append(time)
    queue_df.loc[len(queue_df)] = [time, "Llegada", len(queue), "", "", "", "", service_active]
    ...
    if (time % generate_random_number(break_frequency)) == 0:
        break_start = time + generate_random_number(break_duration)
        queue_df.loc[len(queue_df) - 1, "Hora sig. inicio de descanso"] = break_start
        queue_df.loc[len(queue_df) - 1, "Hora sig. fin de descanso"] = break_start + break_duration
        return break_start, True
    else:
        return time, service_active

# Definir función para manejar salidas
def handle_departure(time, queue, departure_interval, break_frequency, break_duration, service_active):
    """Quita un cliente de la cola cuando el evento es de salida."""
    if len(queue) > 0:
        queue.pop(0)
        queue_df.loc[len(queue_df)] = [time, "Fin de servicio", len(queue), "", "", "", "", service_active]

        # Si hay una pausa en el momento de la salida, se agrega el tiempo de la pausa al próximo evento de salida
        if (time % generate_random_number(break_frequency)) == 0:
            break_start = time + departure_interval + generate_random_number(break_duration)
            queue_df.loc[len(queue_df) - 1, "Hora sig. inicio de descanso"] = break_start
            queue_df.loc[len(queue_df) - 1, "Hora sig. fin de descanso"] = break_start + break_duration
            return break_start, True
        else:
            return time + departure_interval, service_active
    else:
        return time, service_active
...
for t in range(1, queue_duration + 1):  # Salta la primera fila
    if t == next_arrival:
        next_departure, service_active = handle_arrival(t, queue, next_arrival, break_frequency, break_duration, service_active)
        next_arrival += generate_random_number(arr_interval)
    if t == next_departure:
        next_departure, service_active = handle_departure(t, queue, next_departure, break_frequency, break_duration, service_active)
    ...
    queue_df.loc[len(queue_df) - 1, "Estado de servicio"] = "Activo" if service_active else "Inactivo"
...
queue_df["Hora sig. inicio de descanso"] = queue_df["Hora sig. inicio de descanso"].apply(format_time)
queue_df["Hora sig. fin de descanso"] = queue_df["Hora sig. fin de descanso"].apply(format_time)
